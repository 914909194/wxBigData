...
Page({
  ...
  buyCourse:function(){
    ...
    //   title: '购买成功'
    // });
    util.request(api.BuyNow, { userid: that.data.userid, courseid: that.data.courseid, freeprice: that.data.freeprice, price: that.data.price, listimg:that.data.img,coursename:that.data.coursename},'POST').then(function (res) { 
      if (res.errno === 0) {
  ...
  addToCart: function () {
    ...
    util.request(api.CartAdd, { userid: this.data.userid, courseid: this.data.courseid}, "POST").then(function (res) {
      // console.log(res);
      // console.log(res.data.cartTotal.courseCount);
      if (res.errno === 0) {
        that.setData({
          cartCourseCount: res.data.cartTotal.courseCount
        });
